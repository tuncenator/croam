...
> paraphrasing. Skipped or hand-waved entries count as a checkpoint failure
> (the checkpoint agent grep-checks this section against the plan).
...
- **Invocation**: [exact command line, exact code snippet, exact stdio request -- whatever was actually run]
- **Observed outcome**:

  ```
  [Paste the actual response/output/return value byte-for-byte. No paraphrasing,
  no "..." truncation in fields the check inspects.]
  ```

- **Verdict**: [pass | fail -- and if fail, what was wrong and how it was fixed before completing]

### [Check 2, repeat block for each check]

[...]

### Anti-Patterns Watched For

> Reference the anti-patterns from FUNCTIONAL_QA_STRATEGY.md that applied to
> this phase. For each, briefly note how you avoided it (what you specifically
> did NOT do).
...
[Updates or "No strategy updates."]
...
> Functional QA Results above is the structured, per-check evidence; this
> section captures the looser narrative of what else was verified during
> development (external API exploration, ad-hoc smoke testing, log spelunking).
